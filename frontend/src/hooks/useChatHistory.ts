...
export function useChatHistory(conversationIdentifier : string = "unknown"){
    const [chatHistory, setChatHistory] = useState<ChatHistory>(new ChatHistory([], conversationIdentifier))
    ...
    async function fetchMessages() : Promise<void>{
        if(conversationIdentifier === "unknown") return setChatHistory(new ChatHistory([], conversationIdentifier, typing.current))
        const client = getAxiosInstance()
        const lastConversationIdenfier = chatHistory.conversationIdentifier
        try{
            const query = {}
            if(lastConversationIdenfier === conversationIdentifier && chatHistory.lastMessage){
                //@ts-expect-error query['from'] is not a valid property
                query['from'] = chatHistory.lastMessage.identifier
            }
            const queryString = new URLSearchParams(query).toString()
            const response = await client.get(`/conversations/${conversationIdentifier}/messages?${queryString}`)
            ...
            if(lastConversationIdenfier === conversationIdentifier){
                ...
                return setChatHistory(new ChatHistory(messages, conversationIdentifier, typing.current))
            }
        }catch (e) {
            console.error(e)
            return setChatHistory( new ChatHistory([], conversationIdentifier, typing.current))
        }
    }
    ...
    return {chatHistory,fetchMessages,toggleTypingMessage}
}